...
class RecommendationService:
    ...
    async def get_recommendations(self, user_id: str, query: str = None) -> Dict:
        ...
        try:
            ...
            logger.info("Fetching user profile and feedback stats...")
            ...
            logger.info(f"✓ Found {len(similar_products['ids'])} similar products")
            ...
            ai_recommendations = await self.gemini_service.generate_recommendation(
            ...
            logger.error(f"❌ Error in get_recommendations for user {user_id}: {str(e)}", exc_info=True)
            raise
