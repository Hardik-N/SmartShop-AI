...
async def root():
    return {
        "message": "Welcome to SmartShop AI API",
        "version": "1.0",
        "endpoints": {
            "health": "/health",
            "recommendations": "/recommendations/{user_id}?query=optional",
            "feedback": "/feedback (POST)",
            "debug": {
                "users": "/debug/users",
                "products": "/debug/products",
                "generate_data": "/debug/generate-data",
                "check_user": "/debug/check-user/{user_id}"
            },
            "docs": "/docs",
            "redoc": "/redoc"
        }
...
async def generate_test_data(db: Session = Depends(get_db)):
    """
    Quick test data generation endpoint.
    Note: This uses existing vector store from startup. 
    To regenerate vector store, restart the server.
    """
    try:
        ...
        # NOTE: Vector store is already populated from server startup
        # We don't regenerate it here to keep this endpoint fast

        # Return the ID of the first user created
        first_user = db.query(User).first()
        
        return {
            "message": "Test data generated successfully (fast mode - using existing vector store)",
            "test_user_id": first_user.id if first_user else None,
            "stats": {
                "users": len(data["users"]),
                "products": len(data["products"]),
                "behaviors": len(data["user_behaviors"])
            },
            "note": "Vector store from server startup is being used. Restart server to regenerate embeddings."
        }
    except Exception as e:
        db.rollback()
